...
int main(int argc, char *argv[]){
	...
	*****/
	...
	string path = "./rootfiles/"+(string)argv[1]+".root";
	...
	*****/
	...
	// OHNE DEN 1. PEAK! Minima zu Minima, 0. Buckel nicht mitnehmen

	//Range für die Peaks, ablesen an Histogramm.


string _runName=(string)argv[1];
std::vector<int> ranges;
if (strcmp(_runName.c_str(), "calib_vb56") == 0) 
{
   ranges = {2,4,6,8,10,12,14}; 
} 
else if (strcmp(_runName.c_str(), "calib_vb57") == 0)
{
  ranges = {2,4,6,9,12,14,16}; 
}
else if (strcmp(_runName.c_str(), "calib_vb58") == 0)
{
  ranges = {2,5,8,11,14,17,20}; 
}
else if (strcmp(_runName.c_str(), "calib_vb591") == 0)
{
   ranges = {2,6,9,13,16,20,24};
}
else if (strcmp(_runName.c_str(), "calib_vb60") == 0)
{
  ranges = {3,7,11,15,19,23,27}; 
}
else if (strcmp(_runName.c_str(), "calib_vb61") == 0)
{
   ranges = {3,7,11,16,21,26,30}; 
}
else if (strcmp(_runName.c_str(), "calib_vb62") == 0)
{
  ranges = {4,8,12,17,22,27,31}; 
}
else 
{
  //Falscher Name
...
string parent_dir="./calib_histograms/amplitude/";

  string target_dir = parent_dir+(string)argv[1]+"/";
...
if (stat(target_dir.c_str(), &st) == -1) {
    mkdir(target_dir.c_str(), 0700);
 	...
 	string pdf_filename1 = target_dir+(string)argv[1]+"_"+(string)argv[2]+"_new_fit.pdf";
	string pdf_filename2 = target_dir+(string)argv[1]+"_"+(string)argv[2]+"_alt_fit.pdf";
	string pdf_filename3 = target_dir+(string)argv[1]+"_"+(string)argv[2]+"_values.pdf";
  string list_filename = target_dir+"calib_amp_"+(string)argv[1]+".txt";
