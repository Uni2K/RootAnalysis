...
int main(int argc, char *argv[])
{
	...
	*****/
	...
	string wom_id = (string)argv[3];
	string sw_id = (string)argv[4];
	double x_lim = atoi(argv[5]);
	string fileLocation=string(argv[6]);
	...
	string out_list = "BL_values_run_"+run_nr; // filename values list
  	...
  	printf("\nANALYZE CHARGE: run%d, %s, %s\n",run_nr,wom_id.c_str(),sw_id.c_str());	
	...
	*****/
	...
	{	
		...
		*****/
		...
		h_vec[i]->SetMarkerSize(0.5);
		h_vec[i]->SetMarkerColorAlpha(kBlack,0.6);
		if (sw_id=="none"){
			h_vec[i]->SetTitle(Form("baseline fit, red. #chi^{2} , %s, ch%d",wom_id.c_str(),channel));
		}
		else{
			h_vec[i]->SetTitle(Form("baseline fit, red. #chi^{2} , %s, %s, ch%d",wom_id.c_str(),sw_id.c_str(),channel));
		}
		h_vec[i]->GetXaxis()->SetTitle("#chi_{BL}^{2}/ndf");
		...
		*****/
		...
		h_chi2_leg->AddEntry(h_vec[i],Form("#bf{#chi_{BL}^{2}/ndf, baseline fit}"),"elpf");
		h_chi2_leg->AddEntry((TObject*)0,Form("entries = %1.f",h_vec[i]->GetEntries()),"");
		h_chi2_leg->AddEntry(ln_vec[i],Form("local minimum: %1.1f",chi2_min[i]),"l");
		h_chi2_leg->Draw("");
		...
		*****/
		...
		xTitle2.Form("baseline (cut: #chi_{BL}^{2}/ndf<%1.2f) [mV]",chi2_min[i]);
		...
		if (sw_id=="none"){
			h_vec[i+8]->SetTitle(Form("baseline fit, %s, ch%d",wom_id.c_str(),channel));
		}
		else{
			h_vec[i+8]->SetTitle(Form("baseline fit, %s, %s, ch%d",wom_id.c_str(),sw_id.c_str(),channel));
		}
		...
		*****/
		...
		TLegend *h_BL_leg = new TLegend(0.47,0.55,1.0,0.9);
		h_BL_leg->SetTextSize(0.04);
		h_BL_leg->AddEntry(h_vec[i+8],Form("#bf{baseline}"),"elpf");
		h_BL_leg->AddEntry((TObject*)0,Form("entries = %1.f",h_vec[i+8]->GetEntries()),"");
		h_BL_leg->AddEntry(fit_BL_vec[i],Form("Gaussian fit:"),"l");
		h_BL_leg->AddEntry((TObject*)0,Form("mean_{BL} = %1.3f #pm %1.3f",mean_BL[i],mean_BL_err[i]),"");
	...
	*****/
    ...
    {
      pdf_filename1 = out_dir+run_name+Form("_%s_fit.pdf",sw_id.c_str());
      pdf_filename2 = out_dir+run_name+Form("_%s_values.pdf",sw_id.c_str());
    }
