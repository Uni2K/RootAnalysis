...
int main(int argc, char **argv)
{
  ...
  ********************/
  ...
  for (int i = 0; i < n_channel; ++i)
  {
    ...
    if (run_nr==42){ranges = {-20,35,85,135,190,230,270};}
    if (run_nr==50){ranges = {-20,25,75,120,160,200,240};}
    if (run_nr==81){ranges = {-20,30,75,120,160,215,250};}

    // WOM-C/-D,40Array0 sw2
    if (run_nr==41){ranges = {-20,35,85,120,185,220,275};}
    if (run_nr==47){ranges = {-20,25,70,115,165,205,235};}
    if (run_nr==77){ranges = {-20,30,75,120,170,220,260};}

    // WOM-C/-D,40Array0 sw4
    if (run_nr==73){ranges = {-20,40,90,130,175,220,280,315};}
    if (run_nr==84){ranges = {-20,40,90,130,180,220,265,320};}
    if (run_nr==88){ranges = {-20,30,75,110,150,200,220};}

    // WOM-A/-B,8Array0
    if (run_nr==65){ranges = {-20,45,110,160,230,290,345};}
    if (run_nr==68){ranges = {-20,40,105,160,220,280,330};}
    if (run_nr==69){ranges = {-20,45,105,170,240,280,325};}

    // 8ArrayTB17
    if (run_nr==53){ranges = {-25,25,65,110,140,185};}
    if (run_nr==82){ranges = {-20,30,70,115,155,195,210};}

    // TB19 sw5
    if (run_nr==9){ranges = {-20,50,120,160,220,280,330,380,440,500,550,600,660,710,770};}
    ...
    *****/    
    ...
    f->SetParName(0,"N0");f->SetParameter(0,h->Integral("width"));
    f->SetParName(1,"#mu");f->SetParameter(1,3.5);
    f->SetParName(2,"#mu_{XT}");f->SetParameter(2,0.2); f->SetParLimits(2,0,1);
    f->SetParName(3,"#sigma_{0 p.e.}");f->SetParameter(3,par_single[2]); f->SetParLimits(3,0,100);
    f->SetParName(4,"#sigma_{1 p.e.}");f->SetParameter(4,par_single[5]); f->SetParLimits(4,0,100);
    ...
    *****/
    ...
    if (print_verb)
    {
      ...
      for (int j = 0; j < n_peaks; ++j){printf("%1.1f  ",pos_alt[j]);}
      printf("\n");
    }
    if (print_cf_array)
    {
  ...
  {
    printf("calib factor:\n");
    for (int i = 0; i < 8; ++i) {printf("%f,", calib_factor_vec[i]);}
    printf("\n");

    printf("calib factor, statistical error:\n");
    for (int i = 0; i < 8; ++i) {printf("%f,", calib_factor_err_vec[i]);}
    printf("\n");

    printf("calib factor, relative stat+sys error:\n");
    for (int i = 0; i < 8; ++i) {printf("%f,", calib_factor_rel_err_vec[i]);}
    printf("\n");

    printf("baseline offset:\n");
    for (int i = 0; i < 8; ++i) {printf("%f,", BL_offset_vec[i]);}
    printf("\n");

    printf("baseline offset, error:\n");
    for (int i = 0; i < 8; ++i) {printf("%f,", BL_offset_err_vec[i]);}
    printf("\n");
  }

  fprintf(calib_factor_list,"calib factor:\n");
  for (int i = 0; i < 8; ++i) {fprintf(calib_factor_list,"%f,", calib_factor_vec[i]);}
  fprintf(calib_factor_list,"\n");

  fprintf(calib_factor_list,"calib factor, statistical error:\n");
  for (int i = 0; i < 8; ++i) {fprintf(calib_factor_list,"%f,", calib_factor_err_vec[i]);}
  fprintf(calib_factor_list,"\n");

  fprintf(calib_factor_list,"calib factor, relative stat+sys error:\n");
  for (int i = 0; i < 8; ++i) {fprintf(calib_factor_list,"%f,", calib_factor_rel_err_vec[i]);}
  fprintf(calib_factor_list,"\n");

  fprintf(calib_factor_list,"calib fit, reduced chi2:\n");
  for (int i = 0; i < 8; ++i) {fprintf(calib_factor_list,"%f,", calib_fit_rchi2_vec[i]);}
  fprintf(calib_factor_list,"\n");

  fprintf(BL_offset_factor_list,"baseline offset:\n");
  for (int i = 0; i < 8; ++i) {fprintf(BL_offset_factor_list,"%f,", BL_offset_vec[i]);}
  fprintf(BL_offset_factor_list,"\n");

  fprintf(BL_offset_factor_list,"baseline offset, error:\n");
  for (int i = 0; i < 8; ++i) {fprintf(BL_offset_factor_list,"%f,", BL_offset_err_vec[i]);}
  fprintf(BL_offset_factor_list,"\n");
