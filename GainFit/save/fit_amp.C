...
int main(int argc, char *argv[])
{
	...
	*****/
		...
		string pdf_filename1 = target_dir + (string)argv[1] + "_" + buffer + "_GP_fit.pdf";
		string pdf_filename2 = target_dir + (string)argv[1] + "_" + buffer + "_contG_fit.pdf";
		string pdf_filename3 = target_dir + (string)argv[1] + "_" + buffer + "_values.pdf";
		string list_filename = target_dir + "calib_" + (string)argv[1] + ".txt";
		...
		c->DrawClonePad();
		gains_amp[i]=calib_factor;
	}

	string parent_dir = "./calib_histograms/amplitude/";
	string target_dir = parent_dir + (string)argv[1] + "/";
	string overview_filename = target_dir + "overview_" + (string)argv[1] + ".pdf";
	masterCanvas->Print(overview_filename.c_str());

	//Summary
	FILE *summary;
	summary = fopen((parent_dir + "calibration_amp.txt").c_str(), "a");
	string pre = "";

	fprintf(summary, (pre + (string)argv[1] + " = { %f,%f,%f,%f,%f,%f,%f,%f,0};\n").c_str(),gains_amp[0], gains_amp[1], gains_amp[2],gains_amp[3], gains_amp[4], gains_amp[5], gains_amp[6], gains_amp[7]);
