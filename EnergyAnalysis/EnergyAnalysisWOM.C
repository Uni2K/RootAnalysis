...
int main(int argc, char *argv[])
{
	...
	for (const auto &entry : fs::directory_iterator(rootDirectory))
	{
		...
		// export file names
		out_pdf_name = Form("wom_energy_%s.pdf", runName.c_str()); // histogram pdf filename
	...
	*****/
		...
		*****/
			...
			draw_cmnd1.Form("chargeChannelSumWOM[%d]>>h%d", i, i);
			cut_cmnd1.Form("");
...
*/
	...
	{ //Channel Loop
		...
		auto legend = new TLegend(0.1, 0.85, 0.5, 1);
	...
	string saveFinal = generalPrintDir + "//WOMoverview.pdf";
